...
def calc_maximal_hexagonal_distance(directions):
    hexagon = Hexagon(0, 0, 0)
    max_distance = 0
    for direction in map(Direction, directions):
        hexagon = hexagon + direction
        max_distance = max(max_distance, hexagon.distance_from_origin())
...
def main():
    ...
    result = calc_maximal_hexagonal_distance(directions)
    print(result)
